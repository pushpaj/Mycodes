...
void fanScreen(){
  tft.fillScreen(BLUE);
  tft.fillTriangle(20,40+down,35,30+down,35,50+down,RED);
  tft.fillRect(35,35+down,20,10,RED);

  tft.setCursor(10,10);
  tft.println("Home Autoware");
  
  tft.fillCircle(120, 100+down, BUTTONRADIUS, onColor);
  tft.setCursor(115, 90+down);
  tft.println("1");
  
  tft.fillCircle(177, 141.46+down, BUTTONRADIUS, onColor);
  tft.setCursor(172, 131.46+down);
  tft.println("2");
  
  tft.fillCircle(155.3, 208.5+down, BUTTONRADIUS, onColor);
  tft.setCursor(150.3, 198.5+down);
  tft.println("3");

  tft.fillCircle(84.7, 208.5+down, BUTTONRADIUS, onColor);
  tft.setCursor(79.7, 198.5+down);
  tft.println("4");

  tft.fillCircle(63, 141.46+down, BUTTONRADIUS, onColor);
  tft.setCursor(58, 131.46+down);
  tft.println("5");
}
void loop() {
...
//////////////////////////////////////////
 ...
 if ((switch1change == 0 && switch1touch == 0)) {
  ...
  if (!fantouch && fanChange==0) {
    //toggleElements(2);
    //mySerial.write("output2StateOn");
    tft.fillCircle(115, 50, BUTTONRADIUS, onColor);
    tft.setCursor(90, 40);
    tft.println("FAN");
    fanChange = 1;
    delay(200);
  ...
  if (p.z > MINPRESSURE && p.z < MAXPRESSURE) {
    ...
    if (p.x > 115-BUTTONRADIUS && p.x < 115+BUTTONRADIUS) {
      if (p.y > 50-BUTTONRADIUS && p.y < 50+BUTTONRADIUS) {
        fanChange = 0;
        fantouch = !fantouch;
      }
